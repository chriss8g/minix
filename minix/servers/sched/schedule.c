...
#define QUANTUM_TO_LOW_PRIORITY 3 /*the numbers of quantum you need to spend to low your priority*/
...
void balance_queues(void)
{
        ...
        for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
                if (rmp->flags & IN_USE) {
                    if(rmp->quantumFinished == 0 && rmp->priority > rmp->max_priority) {
                            rmp->priority -= 1; /* increase priority */
                            schedule_process_local(rmp);
                    }
                    if(rmp->quantumFinished >= QUANTUM_TO_LOW_PRIORITY && rmp->priority < MIN_USER_Q)  {
                            rmp->priority++;
                            schedule_process_local(rmp);
                    }
                    rmp->quantumFinished = 0;
